...
pub struct AppContainer {
    ...
    database: Arc<dyn dbnexus::pool::ConnectionPool>,
...
impl AppContainer {
    ...
    /// ```
    pub async fn from_config_file(config_path: &str) -> crate::types::Result<Self> {
        // Load configuration
        let config_provider_impl = ConfigProviderImpl::builder()
            .file(config_path)
            .env()
            .build()
            .map_err(|e| crate::types::CoreError::ConfigurationError(e.to_string()))?;
    ...
    pub fn database(&self) -> &Arc<dyn dbnexus::pool::ConnectionPool> {
        &self.database
    }
...
pub struct AppContainerBuilder {
    ...
    database: Option<Arc<dyn dbnexus::pool::ConnectionPool>>,
}
...
impl AppContainerBuilder {
    ...
    pub fn database(mut self, database: Arc<dyn dbnexus::pool::ConnectionPool>) -> Self {
    ...
    pub fn try_build(self) -> crate::types::Result<AppContainer> {
        let config = self.config.ok_or_else(|| {
            crate::types::CoreError::ConfigurationError("config provider is required".to_string())
        })?;
        let cache = self.cache.ok_or_else(|| {
            crate::types::CoreError::ConfigurationError("cache backend is required".to_string())
        })?;
        let database = self.database.ok_or_else(|| {
            crate::types::CoreError::ConfigurationError(
