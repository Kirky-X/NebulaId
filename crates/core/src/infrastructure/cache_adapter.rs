...
/// ```rust,ignore
/// use oxcache::Cache;
/// use nebula_core::infrastructure::CacheAdapter;
/// use std::sync::Arc;
///
...
impl CacheAdapter {
    ...
    pub async fn get<T: DeserializeOwned>(&self, key: &str) -> crate::types::Result<Option<T>> {
        let full_key = self.build_key(key);
        let bytes = self
            .backend
            .get(&full_key)
            .await
            .map_err(|e| crate::types::CoreError::CacheError(e.to_string()))?;

        match bytes {
            Some(data) => {
                let value = serde_json::from_slice(&data).map_err(|e| {
                    crate::types::CoreError::CacheError(format!("Deserialization failed: {}", e))
                })?;
                Ok(Some(value))
            }
            None => Ok(None),
    ...
    pub async fn set<T: Serialize>(
        ...
        let bytes = serde_json::to_vec(value).map_err(|e| {
            crate::types::CoreError::CacheError(format!("Serialization failed: {}", e))
        })?;
    ...
    ///
    /// * `key` - The cache key (without prefix)
    pub async fn delete(&self, key: &str) -> crate::types::Result<()> {
        let full_key = self.build_key(key);
        self.backend
            .delete(&full_key)
            .await
    ...
    ///
    /// * `key` - The cache key (without prefix)
    pub async fn exists(&self, key: &str) -> crate::types::Result<bool> {
        let full_key = self.build_key(key);
        self.backend
            .exists(&full_key)
            .await
    ...
    ///
    /// * `key` - The cache key (without prefix)
    pub async fn ttl(&self, key: &str) -> crate::types::Result<Option<std::time::Duration>> {
        let full_key = self.build_key(key);
        self.backend
            .ttl(&full_key)
            .await
    ...
    pub async fn expire(&self, key: &str, ttl: std::time::Duration) -> crate::types::Result<bool> {
        let full_key = self.build_key(key);
        self.backend
            .expire(&full_key, ttl)
            .await
    ...
    ///
    /// * `key` - The cache key (without prefix)
    pub async fn get_raw(&self, key: &str) -> crate::types::Result<Option<Vec<u8>>> {
        let full_key = self.build_key(key);
        self.backend
            .get(&full_key)
            .await
    ...
    pub async fn clear(&self) -> crate::types::Result<()> {
        self.backend
            .clear()
            .await
            .map_err(|e| crate::types::CoreError::CacheError(e.to_string()))
    ...
    pub async fn len(&self) -> crate::types::Result<u64> {
        self.backend
            .len()
            .await
            .map_err(|e| crate::types::CoreError::CacheError(e.to_string()))
    ...
    pub async fn is_empty(&self) -> crate::types::Result<bool> {
        self.backend
            .is_empty()
            .await
            .map_err(|e| crate::types::CoreError::CacheError(e.to_string()))
