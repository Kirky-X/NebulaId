...
impl ApiHandlers {
    ...
    pub async fn generate(&self, req: GenerateRequest) -> Result<GenerateResponse> {
        ...
        let result = if let Some(ref alg_str) = req.algorithm {
            let algorithm: nebula_core::types::AlgorithmType = alg_str.parse()?;
            self.id_generator
        ...
        let algorithm_name = if let Some(ref alg) = req.algorithm {
            alg.parse::<nebula_core::types::AlgorithmType>()
                .map(|a| a.to_string())
    ...
    pub async fn list_workspaces(&self) -> Result<WorkspaceListResponse> {
        let result =
            self.config_service.list_workspaces().await.map_err(
                |e: nebula_core::types::CoreError| CoreError::DatabaseError(e.to_string()),
            )?;
        Ok(result)
    }
