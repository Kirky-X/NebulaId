...
- `config/config_test.toml` 写的是 `engine = "sqlite"`，而 `sqlite` feature 目前根本
  不可构建：`Cargo.toml` 定义 `sqlite = ["dbnexus/sqlite"]`，默认已开启
  `dbnexus/postgres`，两者同时开启会命中 dbnexus 的 `compile_error!`（"cannot enable both
  client-side (sqlite/duckdb) and server-side (postgres/mysql) database features"）。
  运行时侧 `create_connection` 的 Sqlite 分支
