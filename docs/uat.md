...
### 2.4 场景 4: 跨数据中心部署验证 ⏳ 待验收

**业务背景**:  
公司在北京、上海两地有数据中心，需要验证跨 DC 部署时 ID 全局唯一。
...
1. **部署验证** ⏳ 待验收
   - DC1 (北京): 3 个服务节点
   - DC2 (上海): 3 个服务节点
   - **验收标准**: 两个 DC 服务正常启动，DC_ID 分别为 0 和 1

2. **DC 隔离测试** ⏳ 待验收
   - DC1 生成 10万 个 ID
   - DC2 生成 10万 个 ID
   - 合并后去重
   - **验收标准**: 总数仍为 20万（无重复，ID 区间不重叠）
...
4. **配置同步测试** ⏳ 待验收
   - 在 DC1 管理后台修改配置
   - **验收标准**: DC2 在 5 秒内获取到新配置

**预期结果**: ✅ 跨 DC 部署正常，ID 全局唯一
