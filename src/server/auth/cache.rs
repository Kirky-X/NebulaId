...
//! 认证决策缓存门面 —— garrison `GarrisonDao` 接口的进程内实现。
//!
//! 存储是 garrison `cache-memory` 特性提供的 `GarrisonDaoOxcache`（oxcache
//! L1 内存后端：Moka 并发缓存 + 命中/失效指标 + 键名自动脱敏审计），不再是
//! 自研 HashMap。cache.rs 早期版本曾用 `MemoryGarrisonDao`（无 LRU/上限/
//! 统计），换后端只动了存储构造，门面接口与失效语义保持不变——这正是当初
//! 借用 DAO trait 形状的目的。
//!
...
/// garrison `GarrisonDaoOxcache` 之上的认证缓存门面。
...
impl AuthCache {
    ...
    ///
    /// oxcache L1 内存后端的构建失败（仅内存耗尽类病态场景）在此显性 panic：
    /// 无缓存的降级路径（直连 DB + Argon2id）会静默放大认证延迟，不如启动期
    /// 显性失败。
    ///
    /// # Runtime 约束
    ///
    /// oxcache `_sync` 后端要求 **multi-thread tokio runtime**（`#[tokio::main]`
    /// 默认即满足；current-thread runtime 下其 sync 路径会 panic）——这是
    /// garrison `GarrisonDaoOxcache` 的既有约束，测试需显式
    /// `#[tokio::test(flavor = "multi_thread")]`。
    pub async fn new(ttl_seconds: u64) -> Self {
        let dao = GarrisonDaoOxcache::new()
            .await
            .expect("GarrisonDaoOxcache（oxcache L1 内存后端）初始化不应失败");
