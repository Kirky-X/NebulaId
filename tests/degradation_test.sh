...
echo -e "\n【1】健康检查测试"
echo "----------------------------------------"
health_result=$(curl -s --retry 3 --retry-connrefused "$(get_api_base)/health")
...
echo -e "\n【3】验证指标端点"
echo "----------------------------------------"
metrics_result=$(curl -s --retry 3 --retry-connrefused "$(get_api_base)/metrics")
...
echo -e "\n【7】恢复验证"
echo "----------------------------------------"
final_health=$(curl -s --retry 3 --retry-connrefused "$(get_api_base)/health")
